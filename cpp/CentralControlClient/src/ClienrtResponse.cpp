...
namespace CentralControl
{
    ...
    void CentralControlResponseImpl::ScreenBrightOutResp(byte* cmdParam, bool* isSuccessed)
    {
        grpc::ClientContext context;
        google::protobuf::Empty empty;
        CentralControlResponse::Response5* reply;
        grpc::Status statu = stub_->ScreenBrightOutResp(&context, empty, reply);
        if(statu.ok())
        {
            *cmdParam = strtoul(reply->t1().c_str(), NULL, 16);
            *isSuccessed = reply->b1();
        }
        ShowMessage(statu,"ScreenBrightOutResp");
    }
